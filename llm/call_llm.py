...
def get_completion_gpt(prompt : str, model : str, temperature : float, api_key:str, max_tokens:int):
    # 封装 OpenAI 兼容接口（含 Deepseek 等，通过 OPENAI_API_BASE 指定）
    if api_key == None:
        api_key = parse_llm_api_key("openai")
    base_url = os.environ.get("OPENAI_API_BASE", "https://api.openai.com/v1")
    try:
        _to = float(os.environ.get("LLM_HTTP_TIMEOUT_SEC", "600"))
    except ValueError:
        _to = 600.0
    client = OpenAI(api_key=api_key, base_url=base_url, timeout=_to)
